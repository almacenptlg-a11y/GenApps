...
function initHub(currentUser) {
    ...
    APPS_CATALOG.forEach(app => {
       ...
       // 2. Inyectar Tarjeta Ultra Compacta (App Launcher Style)
        ...
        card.innerHTML = `
            <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[95%] h-[95%] group-hover:top-5 group-hover:-translate-y-0 group-hover:w-40 group-hover:h-40 rounded-full shadow-none group-hover:shadow-md transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] z-20 bg-white flex items-center justify-center overflow-hidden border-2 border-transparent group-hover:border-white">
